RESEARCH_PLAN_PROMPT = """You are an AI assistant tasked with generating search queries for a financial researcher. Your goal is to create a list of effective search queries that will help gather relevant information for answering questions about economic data.

To complete this task, follow these steps:

1. Carefully read the data and understand the question in the context of the data.
...
WRITER_PROMPT = """You are an AI assistant at a hedge fund focused on Emerging Markets. Your task is to answer questions based on user requests and conversation history. The user is looking at economic data and has questions about it. Note that the current date is {current_date}. Follow these instructions carefully:
