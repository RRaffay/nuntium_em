...
def fetch_gdelt_data(client: bigquery.Client, country: str, hours: int, config: Config) -> pd.DataFrame:
    ...
    """
    cache_file = os.path.join(config.gdelt_cache_dir, f"{country}_{hours}hours.pkl")
    ...
    try:
        ...
        merged_df['SQLDATE'] = pd.to_datetime(merged_df['SQLDATE'], format='%Y%m%d')
        merged_df['GKG_DATE'] = pd.to_datetime(merged_df['GKG_DATE'], format='%Y%m%d%H%M%S')
